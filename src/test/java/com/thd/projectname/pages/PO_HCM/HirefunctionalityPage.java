...
public class HirefunctionalityPage extends BasePage {
    private static final Logger log = LogManager.getLogger(HirefunctionalityPage.class);
...
//public WebElement homecountryPreHire;
    ...
    @FindBy(xpath = "//h2[text()='Address']//following::button[@title='Add'][1]")
...
// Job Location----------------------------------------
    ...
    public HirefunctionalityPage() throws IOException {
    }
...
//    phnAddHCC.click();
        WebActionsUtil.explicitWait(driver,phnDeviceHCC,"click");
        workdaycommon.userWait(2000);
        phnDeviceHCC.click();
        workdaycommon.userWait(1000);
        MobileHCC.click();
...
//                log.info("Edit button is present");
        ...
        processProCompHire.click();
        workdaycommon.userWait(3000);
        String overallStatus1 = overallStatusProCompHire.getText();
        String overallStatus = overallStatus1.trim();
        log.info("---------overallStatus------>" + overallStatus);
        workdaycommon.userWait(1000);
        workdaycommon.scroll();
        log.info("scrolled");
        try {
            log.info("inside try");
            if (overallStatus.equals("Successfully Completed")) {
                test.pass("Work Contact change Completed Successfully");
                log.info("Work Contact change Completed Successfully");
            }
        }
        catch(Exception e) {
            log.info("The Overall process is not completed.......Current Status: " + overallStatus);
            test.fail("The Overall process is not completed.......Current Status: " + overallStatus);
            test.pass("Work Contact change not completed successfully");
            driver.quit();
        }
    }
...
//            workdaycommon.setEmployeename(empname1);
            ...
            WebElement Threedots_MngrProxy = driver.findElement(By.xpath("//span[text()='Manager']//following::div[@data-automation-id='promptOption'][2]//following::img[1]"));
            Threedots_MngrProxy.click();
            workdaycommon.scrolltoElement(securityProfile);
            workdaycommon.mousemove(securityProfile,startProxyFromAssociateName);
            workdaycommon.userWait(1000);
            startProxyFromAssociateName.click();
